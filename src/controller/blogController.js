...
let arrManipulation = function (conditionArr) {
    for (let i = 0; i < conditionArr.length; i++) {
      ...
      // Object.values() is used to access the value of "x" OBJECT; since we don't know 
    //   the key(changes according to iteration)
    ...
    try {
      ...
      if (Object.keys(data).length != 0) {
        ...
        if (data.isPublished == true) {
          blogCreated.publishedAt = new Date().toLocaleString();;
  ...
  const getBlogs = async function (req, res) {
    try {
      ...
      //ConditionArr is manipulated in such a way that if values(against respective keys 
    //   in query params) are not entered then that object is eliminated all together from ConditionArr
  ...
  const updateBlog = async function (req, res) {
    try {
      ...
      //The keys which are not present in req.body have their value as undefined and in case of boolean type field it would be false,
    //    so those keys are deleted by running the for-loop for all the key-value pairs of our object "fieldToUpdate"
      ...
      //Because our body and subcategory are an array of string , so we have to push the req.body data into the pre-existing data 
    //   and similiarly in case of body which is string by type we are concatenating the new data to the pre existing string.
      ...
      //If there is a key named isPublished in req.body so we will add a new key named publishedAt in 
    //   our document to get the date and time it is published
  ...
  const deleteBlogsQueryParams = async function (req, res) {
    try {
      ...
      let conditionArr = [
        { category: category },
        { authorId: authorid }, //conditionArr contains authorId present in the token;
        //  user is able to delete only his blogs
        { tags: tagName },
        { subcategory: subcategoryName },
        { isPublished: isPublished },
      ];
  
      //ConditionArr is manipulated in such a way that if values(against respective keys in query params) are not entered then 
    //   that object is eliminated all together from ConditionArr
      arrManipulation(conditionArr);
